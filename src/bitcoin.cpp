...
Options:
  -m, --multiprocess     Run multiprocess binaries qbit-node, qbit-gui.
  -M, --monolithic       Run monolithic binaries qbitd, qbit-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'qbit-qt [ARGS]' or 'qbit-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'qbitd [ARGS]' or 'qbit-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'qbit-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'qbit-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'qbit-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_qbit [ARGS]'.
  chainstate [ARGS] Run qbit kernel chainstate util, equivalent to running 'qbit-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_qbit [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_qbit-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "qbit-gui" : "qbit-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "qbit-node" : "qbitd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("qbit-cli");
            // Since "qbit rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("qbit-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("qbit-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_qbit");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("qbit-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_qbit");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_qbit-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("qbit-util");
        } else {
...
//! Execute the specified qbitd, qbit-qt or other command line in `args`
...
//!             be a relative path to a qbitd, qbit-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/qbit ...
