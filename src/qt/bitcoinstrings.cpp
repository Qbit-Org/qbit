...
QT_TRANSLATE_NOOP("bitcoin-core", "%s request to listen on port %u. This port is considered \"bad\" and thus it is unlikely that any peer will connect to it. See doc/reference/p2p-bad-ports.md for details and a full list."),
...
QT_TRANSLATE_NOOP("bitcoin-core", "Cannot use -reindex on a node that has pruned witness data. Witness-pruned block index flags would be lost, causing data corruption. Use -reindex-chainstate instead, or delete the data directory and perform a fresh sync."),
QT_TRANSLATE_NOOP("bitcoin-core", "Cannot use -reindex-chainstate on a witness-pruned node without -assumevalid=<hash>. Without an assumed-valid checkpoint, historical script validation requires witness data that has been stripped. Pass -assumevalid=<block hash> to skip historical script validation, or delete the data directory and perform a fresh sync."),
QT_TRANSLATE_NOOP("bitcoin-core", "Cannot use -txindex on a node that has pruned witness data. Delete the data directory and perform a fresh archive sync, or restart without -txindex."),
...
QT_TRANSLATE_NOOP("bitcoin-core", "Error: Dumpfile version is not supported. This version of qbit-wallet only supports version 1 dumpfiles. Got dumpfile with version %s"),
...
QT_TRANSLATE_NOOP("bitcoin-core", "P2MR wallet outputs are unavailable before activation height %d (current wallet height: %d)"),
...
QT_TRANSLATE_NOOP("bitcoin-core", "Skipping implicit onion bind on %s because it conflicts with an RPC bind on the same endpoint."),
...
QT_TRANSLATE_NOOP("bitcoin-core", "Witness compaction handoff failed. Will retry on restart."),
QT_TRANSLATE_NOOP("bitcoin-core", "Witness data for blocks after height %d requires validation. Please restart with -reindex."),
QT_TRANSLATE_NOOP("bitcoin-core", "Witness pruning is incompatible with -txindex. Disable -prunewitnesses to keep txindex-backed full transaction history."),
