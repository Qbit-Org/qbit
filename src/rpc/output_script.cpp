...
static RPCHelpMan validateaddress()
{
    return RPCHelpMan{
        ...
        {
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The qbit address to validate"},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                {RPCResult::Type::BOOL, "isvalid", "If the address is valid or not"},
                {RPCResult::Type::STR, "address", /*optional=*/true, "The qbit address validated"},
...
static RPCHelpMan createmultisig()
{
    return RPCHelpMan{
        ...
        "Disabled on chains that enforce P2MR-only outputs.\n",
        {
            ...
            {"address_type", RPCArg::Type::STR, RPCArg::Default{"legacy"}, "The address type to use on unrestricted regtest (-p2mronly=0). Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
        },
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            if (!request.params[2].isNull()) {
                ...
                } else if (parsed.value() == OutputType::BECH32M || parsed.value() == OutputType::P2MR) {
                    throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "createmultisig cannot create bech32m or p2mr multisig addresses");
                }
...
static RPCHelpMan deriveaddresses()
{
    ...
    return RPCHelpMan{
        ...
        "Derives one or more addresses corresponding to an output descriptor.\n"
         ...
         "    mr(pk(<pqc pubkey>))                              P2MR output for a 32-byte PQC pubkey\n"
         "    mr(pk(pqc(<xprv>/87h/.../*)))                     P2MR outputs derived from private BIP32 key material\n"
         "    rawmr(<merkle root>)                              Expert-only P2MR output with the specified 32-byte Merkle root;\n"
         "                                                         the wallet cannot prove or sign arbitrary roots\n"
         "\nFor legacy, segwit, and taproot descriptors, <pubkey> either refers to a fixed public key in hexadecimal notation,\n"
         "or to an xpub/xprv optionally followed by one or more path elements separated by \"/\", where \"h\" represents a hardened child key.\n"
         "For P2MR descriptors, <pqc pubkey> is a 32-byte PQC public key. pqc(<xprv>/...) wraps private BIP32 key material\n"
         "used to derive the committed PQC pubkey; public pqc(xpub/...) descriptors are not a watch-only derivation interface.\n"
         "Use exportpubkeydb/importpubkeydb for watch-only P2MR tracking.\n"
