...
> [!NOTE]
> This is a general wallet reference. For qbit launch-chain address and backup
> guidance, start with [Wallets, P2MR Addresses, and Backups](p2mr-wallets.md).

## 1. Backing Up and Restoring The Wallet

### 1.1 Creating the Wallet

Since version 0.21, qbit no longer has a default wallet.
...
The following command, for example, creates a descriptor wallet. More information about this command may be found by running `qbit-cli help createwallet`.

```
$ qbit-cli createwallet "wallet-01"
```

`qbit rpc` can also be substituted for `qbit-cli`.
...
| -----------------|:------------------------------------------------------------|
| Linux            | `/home/<user>/.qbit/wallets`                                |
| Windows          | `C:\Users\<user>\AppData\Local\Qbit\wallets`                |
| macOS            | `/Users/<user>/Library/Application Support/Qbit/wallets`    |
...
After encrypting the wallet or changing the passphrase, create a new backup
immediately. The keypool is flushed and a new HD seed is generated after
encryption, so any QBT received by the new seed cannot be recovered from the
previous backups. For P2MR signing wallets, also keep backups current after
signing activity because the wallet database stores PQC signature counters.

The wallet's private key may be encrypted with the following command:

```
$ qbit-cli -rpcwallet="wallet-01" encryptwallet "passphrase"
```

Once encrypted, the passphrase can be changed with the `walletpassphrasechange` command.

```
$ qbit-cli -rpcwallet="wallet-01" walletpassphrasechange "oldpassphrase" "newpassphrase"
```
...
```
$ qbit-cli -named createwallet wallet_name="wallet-01" passphrase="passphrase"
```
...
If the wallet is encrypted and the user tries any operation related to private keys, such as sending qbit, an error message will be displayed.

```
$ qbit-cli -rpcwallet="wallet-01" sendtoaddress "<qbit-p2mr-address>" 0.01
error code: -13
error message:
Error: Please enter the wallet passphrase with walletpassphrase first.
```

Use an address generated for the qbit network you are operating on.
...
```
$ qbit-cli -rpcwallet="wallet-01" walletpassphrase "passphrase" 120
```

In the GUI, there is no specific menu item to unlock the wallet. When the user sends QBT, the passphrase will be prompted automatically.
...
```
$ qbit-cli -rpcwallet="wallet-01" backupwallet /home/node01/Backups/backup-01.dat
```
...
If both the wallet and all backups are lost for any reason, the QBT related to this wallet will become permanently inaccessible.

### 1.5 Backup Frequency

The original qbit wallet was a collection of unrelated private keys. If a non-HD wallet had received funds to an address and then was restored from a backup made before the address was generated, then any funds sent to that address would have been lost because there was no deterministic mechanism to derive the address again.

qbit inherits HD wallets with deterministic key derivation, but P2MR signing
wallets also store wallet-local PQC signature counters. Do not treat an old HD
seed backup, descriptor export, xpub, or pubkey database export as a complete
replacement for a current signing-wallet backup.

Back up after wallet creation, encryption or passphrase changes, important
wallet maintenance, and signing activity. Regular backups also preserve
metadata such as labels. Metadata cannot be retrieved from a blockchain rescan,
so if the backup is too old, the metadata will be lost forever.
...
```
$ qbit-cli restorewallet "restored-wallet" /home/node01/Backups/backup-01.dat
```

After that, `getwalletinfo` can be used to check if the wallet has been fully restored.

```
$ qbit-cli -rpcwallet="restored-wallet" getwalletinfo
```
...
Understanding wallet security is crucial for safely storing your QBT. A key aspect is the wallet passphrase, used for encryption. Let's explore its nuances, role, encryption process, and limitations.
