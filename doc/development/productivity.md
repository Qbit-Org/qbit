...
```sh
cmake --build build --target qbitd qbit-cli
cmake --build build --target bitcoin-qt
cmake --build build --target bench_bitcoin
```

The canonical qbit daemon and RPC client targets are `qbitd` and `qbit-cli`,
which produce `build/bin/qbitd` and `build/bin/qbit-cli`. Compatibility targets
named `bitcoind` and `bitcoin-cli` remain for older automation, but new qbit
scripts should use the qbit-named targets and artifacts. You can and should
combine this with `-j`, as above, for a parallel build.
