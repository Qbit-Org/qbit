...
configuration options and toggled while qbitd is running with the `logging`
RPC.  For instance, launching qbitd with `-debug` or `-debug=1` will turn on
...
You can then use the `-debug=lock` configuration option at qbitd startup or
`qbit-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `qbit-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp build/bin/test_qbit
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_qbit --log_level=test_suite
$ valgrind -v --leak-check=full build/bin/qbitd -printtoconsole
...
```shell
llvm-cov show \
    --object=build/bin/test_qbit \
    --object=build/bin/qbitd \
...
To profile a running qbitd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep qbitd` -- sleep 60
```
...
- [Main thread (`qbitd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
There is a tool in `test/lint/git-subtree-check.sh` ([instructions](../../test/lint/README.md#git-subtree-checksh))
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `qbit-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `qbit-cli` and the GUI debug console use this table to determine how to
