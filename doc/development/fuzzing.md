...
## qbit shared-fuzz target map

qbit's frozen shared-fuzz inventory is mapped to concrete harnesses in
`test/fuzz/shared_target_mappings.json`. The direct witness-v2 / P2MR coverage
added for qbit lives in the `p2mr_script` harness:

```sh
$ FUZZ=p2mr_script build_fuzz/bin/fuzz
```

For the main fuzz binary, the CTest smoke check validates the required harness
set from that mapping file:
...
It is possible to specify `qbitd` arguments to the `fuzz` executable.
