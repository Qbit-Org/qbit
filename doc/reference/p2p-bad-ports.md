When qbit automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no qbit node is listening,
or one where a service is listening that is not related to the qbit network.
As a result, this service may occasionally get connection attempts from qbit
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by qbit, trying to
connect because it thinks there is a qbit node on that
...
Below is a list of "bad" ports which qbit avoids when choosing a peer to
...
This list follows inherited bad-port behavior and public browser port-blocking
references:
