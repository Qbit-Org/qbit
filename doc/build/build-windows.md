...
Below are some notes on how to build qbit for Windows.

The options known to work for building qbit on Windows are:
...
package meets the minimum required `g++` version specified in [dependencies.md](/doc/build/dependencies.md).
...
dependencies to install and [dependencies.md](/doc/build/dependencies.md) for a complete overview.
...
Note that for WSL the qbit source path MUST be somewhere in the default mount file system, for
example /usr/src/qbit, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\qbit`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/qbit
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/qbit --strip
```
