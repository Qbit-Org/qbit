...
The headless daemon `qbitd` has the JSON-RPC API enabled by default, the GUI
`qbit-qt` has it disabled by default. This can be changed with the `-server`
option. In the GUI it is possible to execute RPC methods in the Debug Console
Dialog.

## Method Reference

This page documents JSON-RPC transport, endpoints, authentication, and
consistency behavior. The canonical qbit RPC method reference, including
arguments, result schemas, examples, and qbit-specific command semantics, is
published at [docs.qbit.org](https://docs.qbit.org/).
...
This is the endpoint used by qbit-cli when a `-rpcwallet=` parameter is passed in.
...
```sh
# Get block count from the / endpoint when rpcuser=alice and rpcport=8352
$ curl --user alice --data-binary '{"jsonrpc": "2.0", "id": "0", "method": "getblockcount", "params": []}' -H 'content-type: application/json' localhost:8352/

# Get balance from the /wallet/walletname endpoint when rpcuser=alice, rpcport=8352 and rpcwallet=desc-wallet
$ curl --user alice --data-binary '{"jsonrpc": "2.0", "id": "0", "method": "getbalance", "params": []}' -H 'content-type: application/json' localhost:8352/wallet/desc-wallet
...
qbit-cli createwallet mywallet false false "" false false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
qbit-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
qbit-cli -named createwallet mywallet load_on_startup=true
```

`qbit rpc` can also be substituted for `qbit-cli -named`, and is a newer alternative.

## Versioning

The RPC interface might change from one major version of qbit to the
...
The RPC interface allows other programs to control qbit,
including the ability to spend funds from your wallets, affect consensus
verification, read private data, and otherwise perform operations that
can cause loss of money, data, or privacy.  This section suggests how
you should use and configure qbit to reduce the risk that its
RPC interface will be abused.

- **Securing the executable:** Anyone with physical or remote access to
  the computer, container, or virtual machine running qbit can
  compromise either the whole program or just the RPC interface.  This
  includes being able to record any passphrases you enter for unlocking
  your encrypted wallets or changing settings so that your qbit
  program tells you that certain transactions have multiple
  confirmations even when they aren't part of the best block chain.  For
  this reason, you should not use qbit for security sensitive
...
- **Securing local network access:** By default, the RPC interface can
  ...
  an RPC interface on the same port as used by qbit in order to
  trick you into revealing your authentication credentials.  For this
  reason, it is important to only use qbit for
  security-sensitive operations on a computer whose other programs you
  trust.

- **Securing remote network access:** You may optionally allow other
  computers to remotely control qbit by setting the `rpcallowip`
  ...
  Internet.**  Although qbit's RPC interface does use
  ...
  `qbitd -help` for more information about these settings and other
  settings described in this document.

    Related, if you use qbit inside a Docker container, you may
    need to expose the RPC port to the host system.  The default way to
    do this in Docker also exposes the port to the public Internet.
    Instead, expose it only on the host system's localhost, for example:
    `-p 127.0.0.1:8352:8352`

- **Secure authentication:** By default, when no `rpcpassword` is specified, qbit generates unique
  login credentials each time it restarts and puts them into a file
  readable only by the user that started qbit, allowing any of
  that user's RPC clients with read access to the file to login
  automatically. The file is `.cookie` in the qbit data directory by
  default, and using these credentials is the preferred RPC authentication
  method. Set `-rpccookiefile` if you need it somewhere else. If you need to
  generate static login
  credentials for your programs, you can use the script in the
  `share/rpcauth` directory in the qbit source tree.  As a final
