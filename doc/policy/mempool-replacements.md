...
This policy is inherited from upstream Bitcoin Core behavior where qbit has not
changed it. The details below are retained for policy context rather than as
qbit release history.

* Opt-in full replace-by-fee, without inherited signaling, was honored in
  mempool and mining before the current qbit codebase.

* BIP125 was defined based on the upstream implementation.
...
* RBF was enabled by default in the inherited wallet GUI before the current qbit
  codebase.

* Full replace-by-fee is available as a configurable mempool policy.

* Full replace-by-fee is the default policy.
