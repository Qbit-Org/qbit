This directory contains integration tests that test qbitd and the qbit
...
qbitd and qbit-qt by interacting with them through the RPC and P2P
interfaces.
...
Before tests can be run locally, qbit must be built. See the [building instructions](/doc#building) for help.

The following examples assume that the build directory is named `build`.

## Fuzz tests

See [/doc/development/fuzzing.md](/doc/development/fuzzing.md)
...
The P2P and RPC ports used by the qbitd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another qbitd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its qbitd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other qbitd processes are running.

On linux, the test framework will warn if there is another
qbitd process running when the tests are started.

If there are zombie qbitd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
qbitd processes running on the system, so should not be used if any non-test
qbitd processes are being run.**
...
sure qbitd processes are stopped as above):
...
- when run by CI, no logs are output to the console. However, if a test
  fails, the `test_framework.log` and qbitd `debug.log`s will all be dumped
...
`test_framework.log` and qbitd `debug.log`s can be combined into a single
...
call methods that interact with the qbitd nodes-under-test.

If further introspection of the qbitd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/qbitd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/qbitd <pid>
```
