...
class ToolBitcoinTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `qbit node -version` is
        # run on windows, python doesn't capture output from the child
        # `qbitd` and `qbit-node` process started with _wexecvp, and
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through qbit wrapper command with specified arguments."""
    ...
    def run_test(self):
        ...
        self.log.info("Ensure qbit node command invokes qbitd by default")
        self.test_args([], [], expect_exe="qbitd")

        self.log.info("Ensure qbit -M invokes qbitd")
        self.test_args(["-M"], [], expect_exe="qbitd")

        self.log.info("Ensure qbit -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure qbit -m invokes qbit-node")
            self.test_args(["-m"], [], expect_exe="qbit-node")

            self.log.info("Ensure qbit -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="qbit-node")

            self.log.info("Ensure qbit accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="qbit-node")

            self.log.info("Ensure qbit recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="qbit-node")
