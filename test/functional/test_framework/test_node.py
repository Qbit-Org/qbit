...
from .wallet import p2mr_op_true_script
...
QBIT_RESTRICTED_OUTPUT_CHAINS = {"", "main", "test", "testnet3", "testnet4", "signet"}

# The null blocks key (all 0s)
NULL_BLK_XOR_KEY = bytes([0] * NUM_XOR_BYTES)
QBIT_PID_FILENAME_DEFAULT = "qbitd.pid"
...
class TestNode():
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, binaries, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, v2transport=False, uses_wallet=False, ipcbind=False):
        ...
        self.bitcoinconf = self.datadir_path / "qbit.conf"
        ...
        if self.version is not None:
            # Previous-release binaries still default to bitcoin.conf, so point
            # them at the qbit.conf generated by the functional framework.
            self.args.append("-conf=qbit.conf")
    ...
    PRIV_KEYS = [
            # address, raw privkey bytes
            AddressKeyPair('qMWBN1uL5bjW6xaNwCujw1HYM7rxPuwcUE', bytes.fromhex('f5aa8a5961c16ba0eceb0e4b14b1fc7a8b30f4dd6c29d313add97e24d772063f')),
            AddressKeyPair('qVZXbPDXMLYbJwesFAKWhXJnXNsu1zueh5', bytes.fromhex('dc445665f57dff1c3a80ec77d804b7479de009f47774c2c0cfb3976ad5cda1ad')),
            AddressKeyPair('qQrD4LesY5w4ELypZr96EijCwdMxSYmrA6', bytes.fromhex('bb402ae3748aada29bfd5f67f8c273dc9c4cfd4ca9bd04171ec61254858ee827')),
            AddressKeyPair('qTMLgZYicWCo1n9BShUtjQVW8gE466TTy3', bytes.fromhex('f846843a89aeb49022b0b4f9f55ee35d8d753f7187b3950b211d1f25f4dd4135')),
            AddressKeyPair('qVb1U67X1i9q8151zXjrKq3GssA1MDSa9b', bytes.fromhex('4e729c4886bb61fde05f25f1bb3ae7427402f671cd786821c17981c4b23e9438')),
            AddressKeyPair('qeuDmTUXjJhj241XFXjnrtaVjisoQaMVT2', bytes.fromhex('598a35c5a79663684633679fbc55eff58062fe818a757976522333028afe40f7')),
            AddressKeyPair('qc3LFw7YGHDpHBF28mPXgGpjgxzRzJFqQy', bytes.fromhex('52e1fa4824cadfcadaf2b35f99c08b1afe776d11b5ff15d0358a98ffd3cf7a4c')),
            AddressKeyPair('qXpNKZ3UzaQGqnCYYxBER3oETokNe4SfBp', bytes.fromhex('93ad57d2dd71de6fdfbcf8cd55be3d7eb4c54bb062c3dbfe4bfe125c00da8875')),
            AddressKeyPair('qSXYSF8HkNU5varHL26avpK1ZvcE2KXq9i', bytes.fromhex('9bc9bd88410a210df817020fc56953d00c110c88472fbb6cf4e4e0c2b663dc50')),
            AddressKeyPair('qT763MKD8u4otw2VxQ9T8EfWY8jfSSpAKz', bytes.fromhex('0e9967d39758500712c86fd894a212ad05786e9a684c388ac769cc1f93bb37fc')),
            AddressKeyPair('qSHb9CR7Vx2Y3FJdiatVu7rrR9GP3T3oqy', bytes.fromhex('a533d992f90a3008e2b91313790665f455723f8fe7b68a10eadd7dcb8e87380d')),
            AddressKeyPair('qcU4zPEx3tsxVEEHRedu26QJvQHXCxqHN9', bytes.fromhex('3fa5a8bfcf3584d80a6c1f069bfd7a04b2e4dcddb3fe2910b99d81218aff490b')),
    ]

    def get_deterministic_priv_key(self):
        """Return a deterministic priv key in base58, that only depends on the node's index"""
        assert len(self.PRIV_KEYS) == MAX_NODES
        key = self.PRIV_KEYS[self.index]
        return self.AddressKeyPair(key.address, bytes_to_wif(key.key, chain=self.chain))

    def _p2mr_mining_address(self):
        """Return a deterministic P2MR address matching the node's chain."""
        hrp = QBIT_BECH32_HRPS[self.chain]
        script_pubkey = bytes(p2mr_op_true_script())
        assert len(script_pubkey) == 34 and script_pubkey[0] == 0x52 and script_pubkey[1] == 0x20
        witness_program = script_pubkey[2:]
        return encode_segwit_address(hrp, 2, witness_program)

    @staticmethod
    def _chain_config_section(chain):
        return "test" if chain == "testnet3" else chain

    @staticmethod
    def _p2mronly_setting_value(arg, *, chain=None):
        ...
        if "." in key:
            ...
            if chain is not None and section != TestNode._chain_config_section(chain):
                return None
        ...
        enabled = True if not separator else TestNode._argsman_bool_value(value)
        return not enabled if negated else enabled

    @staticmethod
    def _argsman_bool_value(value):
        trimmed = value.strip()
    ...
    def _running_restricted_output_mode(self):
        """Return whether the running node enables regtest-only restricted outputs."""
        restricted = self.chain == "regtest" and self._supports_p2mronly()

        for value in self._iter_p2mronly_config_values():
            restricted = value

        for arg in self.running_args:
            value = self._p2mronly_setting_value(arg, chain=self.chain)
            if value is not None:
                restricted = value
        return restricted

    def _iter_p2mronly_config_values(self):
        active_section = None
        chain_section = self._chain_config_section(self.chain)
        for line in self._read_config_lines():
            option = line.split("#", 1)[0].strip()
            if not option:
                continue
            if option.startswith("["):
                section, separator, _ = option[1:].partition("]")
                if separator:
                    active_section = section.strip()
                continue
            if active_section is not None and active_section != chain_section:
                continue
            value = self._p2mronly_setting_value(option, chain=self.chain)
            if value is not None:
                yield value

    def _read_config_lines(self):
        try:
            if not self.bitcoinconf.is_file():
                return []
            return self.bitcoinconf.read_text(encoding="utf8").splitlines()
        except OSError:
            return []

    def _has_p2mronly_setting(self, args):
        for arg in args:
            if self._p2mronly_setting_value(arg, chain=self.chain) is not None:
                return True

        for _ in self._iter_p2mronly_config_values():
            return True
        return False
    ...
    def _with_regtest_p2mronly_default(self, extra_args):
        ...
        if self.chain == "regtest" and self._supports_p2mronly() and not self._has_p2mronly_setting(extra_args):
            extra_args.append("-p2mronly=0")
        return extra_args

    def _default_generate_address(self):
        if self.chain in QBIT_RESTRICTED_OUTPUT_CHAINS:
            return self._p2mr_mining_address()
        if self.chain == "regtest" and self._running_restricted_output_mode():
            return self._p2mr_mining_address()
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        extra_args = self._with_regtest_p2mronly_default(extra_args)
        self.running_args = list(extra_args)
        ...
        # qbit.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
    ...
    def generate(self, nblocks, maxtries=1000000, **kwargs):
        ...
        return self.generatetoaddress(nblocks=nblocks, address=self._default_generate_address(), maxtries=maxtries, **kwargs)
