...
P2WPKH = 0
P2WSH = 1
WITNESS_INPUT_AMOUNT = Decimal("5")
WITNESS_OUTPUT_AMOUNT = Decimal("4.999")
WITNESS_SETUP_SPEND_COUNT = 5 * 3 * 2 * 2
WITNESS_FUNDING_OUTPUT_COUNT = WITNESS_SETUP_SPEND_COUNT + 4
...
def find_spendable_utxo(node, min_value):
    ...
    if spendable_utxos:
        return min(spendable_utxos, key=lambda utxo: utxo['amount'])
...
class SegWitTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.num_nodes = 3
        self.segwit_activation_height = COINBASE_MATURITY + 165
        # This test tests SegWit both pre and post-activation, so use the normal BIP9 activation.
        self.extra_args = [
            [
                "-acceptnonstdtxn=1",
                f"-testactivationheight=segwit@{self.segwit_activation_height}",
                "-addresstype=legacy",
            ],
            [
                "-acceptnonstdtxn=1",
    ...
    def run_test(self):
        # Preserve the original pre/post-activation relative heights while ensuring spendable coinbases.
        self.generate(self.nodes[0], self.segwit_activation_height - 4)
        ...
        self.nodes[0].sendmany("", {
            self.nodes[0].getnewaddress(): WITNESS_INPUT_AMOUNT
            for _ in range(WITNESS_FUNDING_OUTPUT_COUNT)
        })
        ...
        for _ in range(5):
            for n in range(3):
                for v in range(2):
                    wit_ids[n][v].append(send_to_witness(
                        v,
                        self.nodes[0],
                        find_spendable_utxo(self.nodes[0], WITNESS_INPUT_AMOUNT),
                        self.pubkey[n],
                        False,
                        WITNESS_OUTPUT_AMOUNT,
                    ))
                    p2sh_ids[n][v].append(send_to_witness(
                        v,
                        self.nodes[0],
                        find_spendable_utxo(self.nodes[0], WITNESS_INPUT_AMOUNT),
                        self.pubkey[n],
        ...
        matured_coinbase_height = self.nodes[0].getblockcount() - COINBASE_MATURITY
        matured_coinbase_hash = self.nodes[0].getblockhash(matured_coinbase_height)
        matured_coinbase = self.nodes[0].getblock(matured_coinbase_hash, 2)["tx"][0]
        matured_coinbase_amount = sum((Decimal(str(vout["value"])) for vout in matured_coinbase["vout"]), Decimal("0"))
        assert_equal(
            self.nodes[0].getbalance(),
            balance_presetup - WITNESS_SETUP_SPEND_COUNT * WITNESS_INPUT_AMOUNT + 20 * WITNESS_OUTPUT_AMOUNT + matured_coinbase_amount,
        )
        assert_equal(self.nodes[1].getbalance(), 20 * WITNESS_OUTPUT_AMOUNT)
        assert_equal(self.nodes[2].getbalance(), 20 * WITNESS_OUTPUT_AMOUNT)
        ...
        tmpl = self.nodes[0].getblocktemplate({'rules': ['segwit']})
        assert_equal(tmpl['sizelimit'], 2000000)
        assert_equal(tmpl['weightlimit'], 2000000)
        assert_equal(tmpl['sigoplimit'], 80000)
        assert_equal(tmpl['transactions'][0]['txid'], txid)
        witness_input_count = sum(1 for txin in raw_tx["vin"] if "txinwitness" in txin)
        expected_sigops = 2 * WITNESS_SCALE_FACTOR + witness_input_count
