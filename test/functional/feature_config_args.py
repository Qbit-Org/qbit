...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_dir_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'qbit.conf'
    ...
    def test_default_config_filename(self):
        node = self.nodes[0]
        assert node.bitcoinconf.name == "qbit.conf"

        with node.assert_debug_log(expected_msgs=[f"Config file: {node.bitcoinconf}"]):
            self.start_node(0)
        self.stop_node(0)
    ...
    def test_negated_config(self):
        ...
        ignored_file_message = [f'Data directory "{self.nodes[0].datadir_path}" contains a "qbit.conf" file which is explicitly ignored using -noconf.']
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in qbit.conf or in an included conf file
        ...
        with open(self.nodes[0].datadir_path / 'qbit.conf', 'a', encoding='utf-8') as conf:
    ...
    def test_config_file_log(self):
        ...
        # Write a qbit.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.bitcoinconf.read_text()
        conf_path = default_datadir / "qbit.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a qbit.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "qbit.conf" file which is ignored, because a '
        ...
        # the same qbit.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/qbit.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when qbit.conf in the default data directory sets another datadir '
                      'and it contains a different qbit.conf file that would be ignored')
        ...
        # qbit.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "qbit.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "qbit.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/qbit.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "qbit.conf"
    ...
    def run_test(self):
        ...
        self.test_default_config_filename()
        ...
        conf_file = default_data_dir / "qbit.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_qbit.conf"
