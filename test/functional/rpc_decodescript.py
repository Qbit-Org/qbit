...
class DecodeScriptTest(BitcoinTestFramework):
    ...
    def decodescript_script_pub_key(self):
        ...
        assert_equal(p2a(main=self.chain == "main"), rpc_result['address'])
    ...
    def decodescript_datadriven_tests(self):
        ...
        for script, result in dd_tests:
            result = self.remap_expected_result_for_chain(result)
            rpc_result = self.nodes[0].decodescript(script)
            assert_equal(result, rpc_result)

    def remap_address_for_chain(self, value):
        hrp, separator, _ = value.partition("1")
        if separator:
            witver, witprog = decode_segwit_address(hrp, value)
            if witver is not None:
                return encode_segwit_address(self.active_bech32_hrp, witver, witprog)

        try:
            payload, version = base58_to_byte(value)
        except Exception:
            return value

        if version in {0, 58, 111, 120}:
            return keyhash_to_p2pkh(payload, main=self.chain == "main")
        if version in {5, 63, 196, 125}:
            return scripthash_to_p2sh(payload, main=self.chain == "main")
        return value

    def remap_expected_result_for_chain(self, value):
        if isinstance(value, dict):
            remapped = {key: self.remap_expected_result_for_chain(subvalue) for key, subvalue in value.items()}
            for address_key in ["address", "p2sh", "p2sh-segwit"]:
                if address_key in remapped and isinstance(remapped[address_key], str):
                    remapped[address_key] = self.remap_address_for_chain(remapped[address_key])
            if "desc" in remapped and isinstance(remapped["desc"], str) and remapped["desc"].startswith("addr("):
                addr = remapped["desc"].split("(", 1)[1].split(")", 1)[0]
                remapped["desc"] = descsum_create(f"addr({self.remap_address_for_chain(addr)})")
            return remapped
        if isinstance(value, list):
            return [self.remap_expected_result_for_chain(item) for item in value]
        return value
    ...
    def decodescript_miniscript(self):
        ...
        assert res["segwit"]["desc"].startswith("addr(")
        segwit_addr = res["segwit"]["desc"].split("(", 1)[1].split(")", 1)[0]
        assert_equal(res["segwit"]["desc"], descsum_create(f"addr({segwit_addr})"))
    ...
    def run_test(self):
        self.active_bech32_hrp = self.nodes[0].decodescript("51024e73")["address"].split("1", 1)[0]
