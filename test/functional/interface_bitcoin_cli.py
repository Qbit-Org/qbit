...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"qbit-cli -h\" for help or \"qbit-cli listwallets\" to see "
...
def cli_get_info_find_key(cli_get_info, key_prefix):
    """Find a single -getinfo key with unit suffix '/kvB)' by prefix."""
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Test -getinfo and qbit-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and qbit-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Balance']), initial_balance)
            ...
            paytxfee_key = cli_get_info_find_key(cli_get_info, 'Transaction fee rate (-paytxfee) (')
            relayfee_key = cli_get_info_find_key(cli_get_info, 'Min tx relay fee rate (')
            assert_equal(Decimal(cli_get_info[paytxfee_key]), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info[relayfee_key]), network_info['relayfee'])
            assert_equal(self.nodes[0].cli.getwalletinfo(), wallet_info)

            # Setup to test -getinfo, -generate, and -rpcwallet= with multiple wallets.
            wallets = [self.default_wallet_name, 'Encrypted', 'secret']
            send_amount_1 = (initial_balance * Decimal("0.2")).quantize(Decimal("0.00000001"))
            send_amount_2 = (initial_balance * Decimal("0.6")).quantize(Decimal("0.00000001"))
            ...
            w1.sendtoaddress(w2.getnewaddress(), send_amount_1)
            ...
            # Test qbit-cli -generate with -rpcwallet in multiwallet mode.
