...
fn merge_existing_fields(
    ...
    let mut existing_vectors = Map::new();
    if out_path.exists() {
        let existing_json: Value = serde_json::from_str(&fs::read_to_string(out_path)?)?;
        for existing in existing_json.as_array().into_iter().flatten() {
            if let Some(name) = existing.get("name").and_then(Value::as_str) {
                existing_vectors.insert(name.to_string(), existing.clone());
            }
        }
    }

    let mut merged = Vec::with_capacity(generated.len());
    for vector in generated {
        ...
        let mut object = existing_vectors
            .remove(name)
