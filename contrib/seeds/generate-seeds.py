...
'''
Script to generate the qbit fixed-seed arrays in `src/chainparamsseeds.h`.
...
def process_nodes(g, f, structname):
    ...
    for line in f:
        ...
        if spec is None:  # ignore this entry (e.g. no longer supported addresses like TORV2)
            continue
        blobs.append(bip155_serialize(spec))

    g.write('static constexpr std::array<uint8_t, %d> %s{{\n' % (sum(len(blob) for blob in blobs), structname))
    for blob in blobs:
...
def main():
    ...
    g.write('\n')
    g.write('/**\n')
    g.write(' * List of fixed seed nodes for the qbit network\n')
