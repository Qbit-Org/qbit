# bash programmable completion for qbit-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $qbit-cli for RPC
_qbit_rpc() {
...
_qbit_cli() {
    ...
    # save and use original argument to invoke qbit-cli for -help, help and RPC
    # as qbit-cli might not be in $PATH
    qbit_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($qbit_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_qbit_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
